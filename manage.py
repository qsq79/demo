...
BACK_PATH2="resources"+os.sep+"background.gif"
...
def book_print():
    ...
    if results:
        ...
        for i in range(len(results)):
            val = "库存量不足的书名为：%s"%(results[i][0])
            print2 = tk.Label(root, text = val)
            print2.grid(row = i+1, column = 0, padx = 10, pady = 5)
    else:
        msg._show(title='错误', message='没有库存不足的书籍')
    db.close()
...
def success_tip(id):
    ...
    root2.destroy()
    root1 = tk.Tk()
    root1.title('CUMT图书管理系统')
    #root1.geometry("280x250")
    labe1 = tk.Label(root1, text="欢迎来到CUMT图书管理系统，请选择您要进行的操作：", font=36).grid(row=0, column=0)
